...
You must also update the following entries in `repo-metadata/kde-dependencies/branch-groups.yaml` for your project: `latest-kf6` which should point to the `master` branch, and `stable-kf6` which should point to the desired stable branch.
This ensures that [kde-builder]({{< ref "kde-builder-setup" >}}) can build KDE projects from the correct branches using [branch-groups](https://kde-builder.kde.org/en/configuration/conf-options-table.html#conf-branch-group).
