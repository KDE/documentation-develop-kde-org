...
The `prefix.sh` file is automatically generated by ECM and, using the install prefix we manually set, it temporarily adds (prepends) the newly created "share" location, namely `$HOME/.local/kde/share/`, to [`XDG_DATA_DIRS`](https://specifications.freedesktop.org/basedir-spec/basedir-spec-latest.html), the standard path for application data files, and changes the `PATH` and other Qt environment variables. After that, our application should run as expected.
