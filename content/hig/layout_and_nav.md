...
Fixed-size UI elements, including default and minimum window sizes                    | `gridUnit` (18px) multiplied as needed
...
When using icons that indicate directionality or depict layouts or textual elements, use the reversed versions that are suffixed with `-rtl`. Information on designing reversed icons can be found in the [Icon Localization guidelines]({{< ref "icons/localization" >}})
