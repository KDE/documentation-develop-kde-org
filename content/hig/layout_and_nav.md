...
In medium-sized apps, it's common for a small number of globally-scoped actions to be visible in a “hamburger menu:” a menu that appears when clicking on a button with the `application-menu-symbolic` icon that's located on the app's toolbar. Its contents are static and globally-scoped, with contextually irrelevant items disabled rather than being hidden.
...
This works automatically for items using standard Qt Layout objects such as RowLayout and ColumnLayout and anchor-based positioning, but it's important to check things yourself. For example you can test your app in right-to-left mode by running it in Arabic with `LANGUAGE=ar_AR [app_executable]`. Even if you can't read the words, you can check the layout.
